...
/**
 * Endpoint
 * @name /transactions/leaseCancel
 */
const leaseCancelTxsManyEndpoint = async ctx => {
  const { query } = select(ctx);
  const filters = selectFilters(query);

  ctx.eventBus.emit('ENDPOINT_HIT', {
    url: ctx.originalUrl,
    resolver: 'transactions.leaseCancel.many',
    filters,
  });
...
module.exports = captureErrors(handleError)(leaseCancelTxsManyEndpoint);
