...
>(
  ...
  get: (req: GetRequest) =>
    service.get(req).chain<AppError, Maybe<Response>>((m) =>
      req.decimalsFormat == DecimalsFormat.Long
        ? taskOf<AppError, Maybe<Response>>(m)
        : swapMaybeF<AppError, Response>(
            taskOf,
            m.map((tx) => modifyDecimals([tx]).map((res) => res[0]))
          )
    ),
  mget: (req: MgetRequest) =>
    service.mget(req).chain<AppError, Maybe<Response>[]>((ms) =>
      req.decimalsFormat == DecimalsFormat.Long
        ? taskOf(ms)
        : swapMaybeF<AppError, Response[]>(
            taskOf,
  ...
  search: (req: SearchRequest) =>
    service.search(req).chain<AppError, SearchedItems<Response>>((res) =>
      req.decimalsFormat == DecimalsFormat.Long
        ? taskOf(res)
        : modifyDecimals(res.items).map((txs) => ({
            ...res,
            items: txs,
          }))
    ),
