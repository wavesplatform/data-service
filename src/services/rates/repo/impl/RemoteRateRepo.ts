...
export default class RemoteRateRepo implements AsyncGet<RateMgetParams, RateInfo[], DbError> {

  constructor(
    private readonly pgp: PgDriver
  ) { }

  get(request: RateMgetParams): Task<DbError, RateInfo[]> {
    const pairsSqlParams = chain(it => [it.amountAsset, it.priceAsset], request.pairs)

    const sql = pg.raw(
      makeSql(request.pairs.length),
      [ request.timestamp.getOrElse(new Date()), request.matcher, ...pairsSqlParams ]
    );
