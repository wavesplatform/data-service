...
const filterByAliases = (aliasSet: string[]) =>
  pg({ t: 'txs_10' }).select('t.tx_uid').whereIn('t.alias', aliasSet);
...
export default {
  get: (alias: string) =>
    selectFiltered(filterByAliases([alias]))
      ...
      .toString(),
  mget: (aliases: string[]) =>
    selectFiltered(filterByAliases(aliases)).select(columns).clone().toString(),
  search: ({ address, showBroken, after, limit }: AliasesSearchRequest) => {
    const q = pg('aliases_cte')
      .with(
        'aliases_cte',
        selectFiltered(
          pg('txs_10').select('tx_uid').where('sender', address)
